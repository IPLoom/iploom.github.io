...
IPLoom integrates natively with Home Assistant via **MQTT Discovery**, automatically registering every device on your network as a trackable entity — no manual YAML configuration required.
...
When a device is discovered (or its status changes), IPLoom publishes two things to your MQTT broker:
...
- MQTT must be **enabled and connected** in IPLoom Settings. See the [MQTT Integration Guide](./mqtt_integration.md).
...
```
unique_id = iploom_{mac_without_colons_lowercase}
# Example: MAC 00:11:22:AA:BB:CC → iploom_001122aabbcc
```
...
|---|---|---|
| `homeassistant/device_tracker/{unique_id}/config` | IPLoom → HA | Discovery config — creates the entity in HA. Published with `retain=true`. |
| `{base_topic}/devices/{unique_id}/status` | IPLoom → HA | Device presence: `online` or `offline`. Published with `retain=true`. |
| `{base_topic}/devices/{unique_id}/attributes` | IPLoom → HA | JSON object with device metadata. Published with `retain=true`. |

### Example Topics (with default base topic)

```
homeassistant/device_tracker/iploom_001122aabbcc/config
network_scanner/devices/iploom_001122aabbcc/status
network_scanner/devices/iploom_001122aabbcc/attributes
```
...
When a device comes online, IPLoom publishes the following config payload to the discovery topic:

```json
{
  "name": "My iPhone",
  "unique_id": "iploom_001122aabbcc",
  "state_topic": "network_scanner/devices/iploom_001122aabbcc/status",
  "json_attributes_topic": "network_scanner/devices/iploom_001122aabbcc/attributes",
  "payload_home": "online",
  "payload_not_home": "offline",
  "icon": "mdi:cellphone",
  "device": {
    "identifiers": ["iploom_001122aabbcc"],
    "name": "My iPhone",
...
> **Entity Type**: IPLoom registers devices as **`device_tracker`** entities (not `binary_sensor`). This means they appear in the **People** and **Presence** sections of Home Assistant and can be used for person tracking automations.
...
| `scanner` | `"IPLoom"` | Source identifier |
...
IPLoom publishes device state on every **scan cycle** when a device status changes:
...
No YAML is required. After IPLoom publishes the discovery payload, Home Assistant will:
...
Device icons are automatically mapped from the IPLoom classification engine to Material Design Icons (`mdi:`) recognized by Home Assistant. Examples:

| IPLoom Device Type | Icon |
|---|---|
...
- **Entities not appearing in HA**: Verify the MQTT broker is shared between IPLoom and HA. Use [MQTT Explorer](https://mqtt-explorer.com/) to watch the `homeassistant/#` topic for incoming discovery payloads.
- **Entity shows "Unavailable"**: The state topic may not have been published yet. Trigger a manual scan in IPLoom to force a re-publish.
- **Wrong entity name**: IPLoom uses the device **hostname** as the entity name. Update the hostname in **IPLoom → Device Details** and it will be reflected on the next scan.
