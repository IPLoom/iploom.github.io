...
Integrate your AdGuard Home instance with HNMS to unlock DNS-level analytics, per-device query tracking, and ad/tracker blocking visibility across your entire network.

## How it Works

HNMS uses a **"Pull" model**, periodically connecting to the AdGuard Home REST API (`/control/querylog`, `/control/stats`) to fetch DNS query logs. By default, it polls every **5 minutes** (configurable).
...
- **Per-Device Analytics**: Maps DNS client IPs to known HNMS devices and aggregates **24-hour query and block counts**.
...
> **Note:** HNMS maps DNS clients using device **IP address**. Devices must be discovered by the Network Scanner first for their DNS stats to be attributed correctly. Unrecognised client IPs are still logged but appear as anonymous entries.

---

## Prerequisites

- AdGuard Home **v0.107+** running on your network (local LAN access from the HNMS host is sufficient).
...
3. Set the retention period to at least **7 days** (matches HNMS's default retention).
...
1. Open HNMS and navigate to **Settings**.
...
```mermaid
sequenceDiagram
    participant HNMS Backend
    participant AdGuard Home API
    participant DNS DB (dns_logs.duckdb)
    participant Main DB (devices)

    HNMS Backend->>AdGuard Home API: GET /control/querylog?limit=1000
    AdGuard Home API-->>HNMS Backend: [ { time, client, question, status, elapsedMs } ]
    HNMS Backend->>HNMS Backend: Filter entries newer than last_sync cursor
    HNMS Backend->>DNS DB (dns_logs.duckdb): Upsert domains, insert dns_logs rows
    HNMS Backend->>AdGuard Home API: GET /control/stats
    AdGuard Home API-->>HNMS Backend: { num_dns_queries, num_blocked_filtering, ... }
    HNMS Backend->>DNS DB (dns_logs.duckdb): Aggregate 24h stats per device_id
    HNMS Backend->>Main DB (devices): UPDATE dns_stats (queries_24h, blocked_24h, last_activity)
    HNMS Backend->>DNS DB (dns_logs.duckdb): DELETE dns_logs older than 7 days
```
...
- **Status stays "Not Verified"**: Confirm the URL is reachable from the HNMS host and the username/password are correct. Try the `curl` command from the HNMS server.
...
- **Only Getting Partial Data**: HNMS fetches a maximum of **1,000 entries per sync**. On very busy networks, increase the sync frequency (lower the interval) to avoid missing queries between cycles.
